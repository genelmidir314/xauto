...
async function translateToTurkish(text) {
  ...
  if (!input) return "";
  if (looksTurkish(input)) return input;
  if (!OPENAI_API_KEY || !OPENAI_KEY_VALID) return input;
  try {
    const out = await openaiChat(
      `You translate tweets into Turkish. Produce a natural Turkish tweet. Rules: Remove RT prefix, remove t.co links, keep meaning identical, max ~200 chars, output ONLY the Turkish tweet. If already Turkish return unchanged.`,
      input,
      0.2
    );
    return String(out || "").trim() || input;
  } catch (e) {
    console.warn("translateToTurkish:", e?.message);
    return input;
  }
}
...
async function generateComment(sourceHandle, originalText, translationTr, hasMedia) {
  ...
  try {
    const mediaHint = hasMedia ? "Bu tweet medya/video içeriyor." : "Bu tweet medya içermiyor.";

    const out = await openaiChat(
      `Sen X (Twitter) platformunda aktif olan gerçek bir kullanıcı gibi davranan bir AI'sın.

Amacın tweetlere kısa, doğal ve merak uyandıran Türkçe yorumlar yazmak.
...
- Hashtag kullanma
- Link kullanma
- Aynı yorumları tekrar etme
- İnsan tepkisi gibi yaz
...
Yorumların amacı:
- İnsanların cevap yazmasını sağlamak
- Tartışma başlatmak
- Merak uyandırmak

Tweeti analiz et: konu (spor/komik/ilginç/olay/hayvan/dans/insan hikayesi), duygu (komik/şaşırtıcı/garip/etkileyici), video var mı, en uygun tepki tipi. Sonra doğal bir Türkçe yorum yaz.

Tweet video içeriyorsa: videonun sonuna dikkat çek, merak uyandır, kısa tepki ver, insanların videoyu tekrar izlemesini sağlayacak yorum yaz.

Kalıpları çeşitlendir: yok artık, nasıl oldu, inanamadım, bu gerçek mi, devamı var mı gibi.

Eğer tweet siyasi, hassas veya tartışmalı ise sadece "..." yaz.

Output: Sadece Türkçe yorum cümlesi (veya yorum yazamazsan "...").`,
      `
Kaynak: @${sourceHandle}
Orijinal tweet:
${cleanOriginal}
...
Tweeti oku ve yukarıdaki kurallara göre kısa, doğal bir Türkçe yorum yaz.
...
Yorum:`,
    ...
    console.log("OPENAI RESULT [comment]:", trimmed);
    return trimmed;
  } catch (e) {
...
async function generateHashtags(commentTr, translationTr, originalText) {
  const cleanComment = cleanupTweetText(commentTr || "");
  const cleanTranslation = cleanupTweetText(translationTr || "");
  const cleanOriginal = cleanupTweetText(originalText || "");
  const context = [cleanComment, cleanTranslation, cleanOriginal].filter(Boolean).join("\n\n");

  if (!context) return fallbackHashtags();
  ...
  try {
    const out = await openaiChat(
      `You generate 2-4 relevant Turkish hashtags for a social media post.

Rules:
- Output ONLY hashtags, space-separated (e.g. #Teknoloji #AI #Gundem)
- Each hashtag must start with #
- 2-4 hashtags total
- Hashtags must be relevant to the content
- Use common Turkish/X hashtags when appropriate
- No explanations, no other text`,
      `Content:\n${context.slice(0, 500)}`,
      0.5
    );

    const tags = String(out || "").match(/#[\w\u00C0-\u024F]+/g) || [];
    if (tags.length > 0) return tags.join(" ").slice(0, 80);
    return fallbackHashtags();
  } catch (e) {
    console.log("⚠️ OpenAI hashtag exception:", e.message);
    return fallbackHashtags();
  }
}
